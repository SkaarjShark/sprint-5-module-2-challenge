    ...
    for (let m = 0; m < 5; m++) {
      ...
      square.addEventListener('click', () => {
        ...
        allSquares.forEach(sq => {
          if (sq.classList.contains("targeted")) sq.classList.remove("targeted")
        })
  ...
  document.addEventListener('keydown', evt => {
    ...
    if (evt.key === 'ArrowUp') {
      if (targeted.parentElement.previousElementSibling) {
        idx = Array.from(targeted.parentElement.children).indexOf(targeted)
    ...
    if (evt.key === ' ') {
      ...
      if (!liveMosquitoes.length) {
        let elasped = getTimeElapsed() 
        document.querySelector("p.info").textContent = `Extermination completed in ${elasped/1000} seconds!`
        // Restart Button
